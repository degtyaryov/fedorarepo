...
%define majorversion 1.2.1
%define packageversion 121
...
Source0:	http://downloads.sourceforge.net/project/postgres-xc/Version_%{majorversion}/pgxc-v%{version}-git.tar.gz
%if !%systemd
Source1:	%{oname}.init
%endif
...
%endif
%if %systemd
Source18:       %{oname}-%{majorversion}.service
Source19:	%{oname}-gtm-%{majorversion}.service
%endif
...
%description libs
The postgresxc%{packageversion}-libs package provides the essential shared libraries for any
...
The postgresxc%{packageversion}-server package includes the programs needed to create and run
...
The postgresxc%{packageversion}-gtm package includes the programs needed to create and run a
...
%description docs
The postgresxc%{packageversion}-docs package includes the SGML source for the Postgres-XC
...
%description contrib
The postgresxc%{packageversion}-contrib package contains contributed packages that are
...
%description devel
The postgresxc%{packageversion}-devel package contains the header files and libraries needed
...
postgresxc%{packageversion}-plperl package contains the PL/Perl language for the backend.
%endif
...
Postgres-XC is an open source project to provide a write-scalable, synchronous
multi-master, transparent PostgresSQL cluster solution. The
postgresxc%{packageversion}-plpython package contains the PL/Python language for the backend.
%endif
...
install -m 644 %{SOURCE18} %{buildroot}%{_unitdir}/%{oname}-%{majorversion}.service
install -m 644 %{SOURCE19} %{buildroot}%{_unitdir}/postgresgtm-%{majorversion}.service
%else
...
if [ $1 = 0 ] ; then
%if %systemd
    /bin/systemctl --no-reload disable %{oname}-%{majorversion}.service >/dev/null 2>&1 || :
    /bin/systemctl stop %{oname}-%{majorversion}.service >/dev/null 2>&1 || :
%else
...
if [ $1 -ge 1 ]; then
%if %systemd
    /bin/systemctl try-restart %{oname}-%{majorversion}.service >/dev/null 2>&1 || :
%else
...
if [ $1 = 0 ] ; then
%if %systemd
    /bin/systemctl --no-reload disable postgresgtm-%{majorversion}.service >/dev/null 2>&1 || :
    /bin/systemctl stop postgresgtm-%{majorversion}.service >/dev/null 2>&1 || :
%else
...
if [ $1 -ge 1 ]; then
%if %systemd
    /bin/systemctl try-restart postgresgtm-%{majorversion}.service >/dev/null 2>&1 || :
%else
...
%{_sbindir}/update-alternatives --install /usr/share/man/man1/pgxc_clean.1   %{sname}-pgxc_cleanman    %{pgbaseinstdir}/share/man/man1/pgxc_clean.1   %{priority}
%{_sbindir}/update-alternatives --install /usr/share/man/man1/pgxc_ctl.1     %{sname}-pgxc_ctlman      %{pgbaseinstdir}/share/man/man1/pgxc_ctl.1     %{priority}
%{_sbindir}/update-alternatives --install /usr/share/man/man1/pgxc_ddl.1     %{sname}-pgxc_ddlman      %{pgbaseinstdir}/share/man/man1/pgxc_ddl.1     %{priority}
%{_sbindir}/update-alternatives --install /usr/share/man/man1/pgxc_monitor.1 %{sname}-pgxc_monitorman  %{pgbaseinstdir}/share/man/man1/pgxc_monitor.1 %{priority}
...
%postun
  if [ "$1" -eq 0 ]
   then
    ...
    %{_sbindir}/update-alternatives --remove %{sname}-pgxc_cleanman     %{pgbaseinstdir}/share/man/man1/pgxc_clean.1
    %{_sbindir}/update-alternatives --remove %{sname}-pgxc_ctlman       %{pgbaseinstdir}/share/man/man1/pgxc_ctl.1
    %{_sbindir}/update-alternatives --remove %{sname}-pgxc_ddlman       %{pgbaseinstdir}/share/man/man1/pgxc_ddl.1
    %{_sbindir}/update-alternatives --remove %{sname}-pgxc_monitorman   %{pgbaseinstdir}/share/man/man1/pgxc_monitor.1
...
%{pgbaseinstdir}/share/man/man1/pgxc_clean.*
%{pgbaseinstdir}/share/man/man1/pgxc_ctl.*
%{pgbaseinstdir}/share/man/man1/pgxc_ddl.*
%{pgbaseinstdir}/share/man/man1/pgxc_monitor.*
...
%{pgbaseinstdir}/share/man/man1/pg_xlogdump.*
...
%defattr(-,root,root)
%if %systemd
%{_unitdir}/%{oname}-%{majorversion}.service
%else
...
%defattr(-,root,root)
%if %systemd
%{_unitdir}/postgresgtm-%{majorversion}.service
%else
...
%changelog
* Thu Apr 9 2015 Degtyaryov Dmitriy <degtyaryov@gmail.com> - 1.2.1-3PGDG
- Latest git version

* Mon Mar 30 2015 Degtyaryov Dmitriy <degtyaryov@gmail.com> - 1.2.1-2PGDG
- Add systemd support

* Thu Mar 26 2015 Degtyaryov Dmitriy <degtyaryov@gmail.com> - 1.2.1-1PGDG
- Update to 1.2.1.
